/**
 * Built-in CapabilityProfile catalogue — the CTF "specialist" preset set.
 *
 *   - orchestrator  Strategic command: scheduling, handoff approval, NO direct
 *                  execution tools (nmap / curl / RsaCtfTool etc).
 *   - triage        Low-cost image / file / network heuristics, NO deep solving.
 *   - image-stego   PNG/JPEG-only tools. Default workflow: image_quick_scan.
 *   - crypto        crypto-only tools. Default workflow: encoding_sweep,
 *                  rsa_common_attacks.
 *   - file-forensics  archive + magic + entropy. Default: unknown_file_triage,
 *                  archive_recursive_extract.
 *
 * The id of each profile matches the SpecialistAgentFactory convention. Adding
 * a profile is one entry here + register a matching set of tools.
 */

import type { CapabilityProfile } from '../core/capabilityProfile.js'

export const PROFILES: Record<string, CapabilityProfile> = {
  orchestrator: {
    id: 'orchestrator',
    displayName: 'CTF Orchestrator',
    description: '全局指挥 Agent。只负责任务拆分 / 调度 / 接力 / 取消。它不直接执行低级工具。',
    systemPromptModules: ['role.boundary', 'tool.first', 'handoff.protocol', 'artifact.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'WebFetch',
      'WebSearch',
      'TodoWrite',
      'load_skill',
      'memory_search',
      'memory_recall',
      'handoff_request',
      'request_specialist',
      'cancel_specialist',
      'update_priority',
      'list_artifacts',
      'list_findings',
      'list_jobs',
    ],
    deniedTools: ['Bash', 'Write', 'Edit', 'Agent', 'TmuxSession'],
    allowedWorkflows: [],
    deniedWorkflows: [],
    allowShell: false,
    allowPython: false,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    limits: { maxIterations: 50 },
  },
  triage: {
    id: 'triage',
    displayName: 'Triage Agent',
    description: '低成本初筛 Agent:识别输入类型 / 启发性检查 / 路由建议。它不做深度解题。',
    systemPromptModules: ['role.boundary', 'tool.first', 'triage.heuristics'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'WebFetch',
      'WebSearch',
      'load_skill',
      'memory_search',
      'memory_recall',
      'emit_finding',
      'request_handoff',
    ],
    deniedTools: ['Write', 'Edit'],
    allowedWorkflows: ['unknown_file_triage'],
    allowShell: true,
    allowPython: false,
    allowBackgroundJobs: false,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['image-stego', 'crypto', 'file-forensics'],
    limits: { maxIterations: 30, maxToolCalls: 80 },
  },
  'image-stego': {
    id: 'image-stego',
    displayName: 'Image Stego Agent',
    description: 'PNG / JPEG / GIF / BMP 等图片隐写 Agent。仅暴露图片相关工具和工作流。',
    systemPromptModules: ['role.boundary', 'tool.first', 'image.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'load_skill',
      'memory_search',
      'memory_recall',
      'emit_finding',
      'request_handoff',
      'extract_artifact',
      'zsteg',
      'binwalk',
      'exiftool',
      'pngcheck',
      'identify',
      'steghide',
      'strings',
      'file',
      'qr_decode',
      'channel_analyze',
      'jpeginfo',
      // §Round-3 — forensics extraction tools referenced by
      // forensics_png_after_end, forensics_bmp_lsb, and forensics_unzip
      // workflows. Without these, the broker denies the tool calls.
      'png_after_iend',
      'bmp_lsb_extract',
      'unzip_inner',
      'list_artifacts',
      'list_findings',
      'list_jobs',
      'query_background_job',
      'collect_background_result',
      'inspect_artifact_summary',
      'inspect_finding',
    ],
    deniedTools: ['nmap', 'sqlmap', 'gdb', 'tshark'],
    allowedWorkflows: [
      'image_quick_scan',
      'forensics_png_after_end',
      'forensics_bmp_lsb',
      'forensics_unzip',
      'png_stego_sweep',
      'jpeg_stego_sweep',
      'image_embedded_file_scan',
    ],
    deniedWorkflows: ['host_service_enumeration'],
    allowShell: true,
    allowPython: false,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['crypto', 'file-forensics'],
    limits: { maxIterations: 60, maxToolCalls: 200 },
  },
  crypto: {
    id: 'crypto',
    displayName: 'Crypto Agent',
    description: 'Crypto / encoding 题目 Agent。优先 encoding_sweep / rsa_common_attacks。',
    systemPromptModules: ['role.boundary', 'tool.first', 'crypto.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'load_skill',
      'memory_search',
      'memory_recall',
      'Python',
      'emit_finding',
      'request_handoff',
      'extract_artifact',
      'rsactftool',
      'yafu',
      'openssl-rsa',
      'binwalk',
      'exiftool',
      'hashcat',
      'john',
      'sage',
      'cyberchef',
      // §13 R1 fix — the recursive multi-layer codec decoder referenced by
      // encoding_sweep. Without this, the broker denies with "not in profile"
      // and the workflow fails even though the tool is registered globally.
      'decode_tree',
      'xor_known_plaintext',
      'aes_ecb_decrypt',
      'rsa_wiener_attack',
      'base64_decode',
      'base64_encode',
      'hex_decode',
      'hex_encode',
      'url_decode',
      'url_encode',
      'response_diff',
      'jsfuck_encode',
      'php_filter_chain',
      'rot13',
      'morse_decode',
      'bacon_cipher',
      'vigenere',
      'multi_byte_xor',
      'rail_fence',
      'base85',
      'binary_decode',
      'braille',
      'list_artifacts',
      'list_findings',
      'list_jobs',
      'query_background_job',
      'collect_background_result',
      'inspect_artifact_summary',
      'inspect_finding',
    ],
    deniedTools: ['nmap', 'tshark'],
    allowedWorkflows: [
      'encoding_sweep',
      'classical_cipher_sweep',
      'xor_key_search',
      'xor_known_attack',
      'aes_ecb_attack',
      'rsa_wiener_attack',
      'rsa_common_attacks',
      'hash_identify_and_crack',
    ],
    deniedWorkflows: ['host_service_enumeration'],
    allowShell: true,
    allowPython: true,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['file-forensics'],
    limits: { maxIterations: 60, maxToolCalls: 250 },
  },
  'file-forensics': {
    id: 'file-forensics',
    displayName: 'File Forensics Agent',
    description: '文件取证 Agent:压缩包 / 魔数修复 / 嵌套文件 / 元数据。',
    systemPromptModules: ['role.boundary', 'tool.first', 'forensics.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'load_skill',
      'memory_search',
      'memory_recall',
      'emit_finding',
      'request_handoff',
      'extract_artifact',
      'binwalk',
      'exiftool',
      'binary_decode',
      'braille',
      'list_artifacts',
      'list_findings',
      'list_jobs',
      'query_background_job',
      'collect_background_result',
      'inspect_artifact_summary',
      'inspect_finding',
    ],
    deniedTools: ['nmap', 'tshark', 'sqlmap'],
    allowedWorkflows: [
      'unknown_file_triage',
      'archive_recursive_extract',
      'embedded_content_scan',
      'magic_header_repair_candidates',
    ],
    deniedWorkflows: ['host_service_enumeration'],
    allowShell: true,
    allowPython: false,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['crypto', 'image-stego'],
    limits: { maxIterations: 60, maxToolCalls: 200 },
  },
  reverse: {
    id: 'reverse',
    displayName: 'Reverse Engineering Agent',
    description:
      '二进制逆向 Agent:静态分析( file/strings/nm/objdump/r2 )+ 动态调试( gdb )+ 跨页结构化输出。',
    systemPromptModules: ['role.boundary', 'tool.first', 'reverse.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'load_skill',
      'memory_search',
      'memory_recall',
      'emit_finding',
      'request_handoff',
      'extract_artifact',
      'file',
      'strings',
      'objdump',
      'nm',
      'radare2',
      'gdb',
      'exiftool',
      // §Round-3 — xor_single_byte + atbash tools used by
      // reverse1 / reverse2 workflows.
      'xor_single_byte',
      'atbash',
      'reverse_elf_decrypt',
      'list_artifacts',
      'list_findings',
      'list_jobs',
      'query_background_job',
      'collect_background_result',
      'inspect_artifact_summary',
      'inspect_finding',
    ],
    deniedTools: ['nmap', 'sqlmap', 'tshark'],
    allowedWorkflows: ['binary_triage', 'function_disassembly', 'embedded_string_search', 'pcap_grep_flag', 'xor_single_byte', 'atbash', 'reverse_elf'],
    deniedWorkflows: ['host_service_enumeration'],
    allowShell: true,
    allowPython: false,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['crypto', 'file-forensics', 'pwn'],
    limits: { maxIterations: 80, maxToolCalls: 300 },
  },
  pwn: {
    id: 'pwn',
    displayName: 'Pwn / Exploit Agent',
    description:
      '漏洞利用 Agent:checksec + gdb 调试 + python3 payload + 段表分析。允许 Python 脚本编写 exploit。',
    systemPromptModules: ['role.boundary', 'tool.first', 'pwn.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'load_skill',
      'memory_search',
      'memory_recall',
      'Python',
      'emit_finding',
      'request_handoff',
      'extract_artifact',
      'file',
      'strings',
      'objdump',
      'gdb',
      'exiftool',
      // §Round-3 — pcap_grep_flag + xor_single_byte + atbash workflows
      // share grep_for_flag / xor_single_byte / atbash tools.
      'grep_for_flag',
      'xor_single_byte',
      'atbash',
      'reverse_elf_decrypt',
      'list_artifacts',
      'list_findings',
      'list_jobs',
      'query_background_job',
      'collect_background_result',
      'inspect_artifact_summary',
      'inspect_finding',
    ],
    deniedTools: ['nmap', 'sqlmap', 'tshark'],
    allowedWorkflows: ['pwn_triage', 'register_state_capture', 'segfault_backtrace', 'pcap_grep_flag', 'xor_single_byte', 'atbash'],
    deniedWorkflows: ['host_service_enumeration'],
    allowShell: true,
    allowPython: true,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['reverse', 'crypto'],
    limits: { maxIterations: 100, maxToolCalls: 400 },
  },
  web: {
    id: 'web',
    displayName: 'Web Exploitation Agent',
    description:
      'Web 漏洞 Agent:服务发现( nmap 后台 )+ 路径枚举( gobuster )+ 漏洞扫描( nikto/sqlmap )+ HTTP 操作( curl / httpx / whatweb )。',
    systemPromptModules: ['role.boundary', 'tool.first', 'web.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'WebFetch',
      'load_skill',
      'memory_search',
      'memory_recall',
      'emit_finding',
      'request_handoff',
      'extract_artifact',
      'curl',
      'nmap',
      'nikto',
      'sqlmap',
      'gobuster',
      'httpx',
      'fingerprint',
      // §Round-3 — generic HTTP fetch + grep-for-flag used by
      // web1 / web_sqli / pcap1 / pcap_http dispatch paths.
      'web_fetch',
      'grep_for_flag',
      'list_artifacts',
      'list_findings',
      'list_jobs',
      'query_background_job',
      'collect_background_result',
      'inspect_artifact_summary',
      'inspect_finding',
    ],
    deniedTools: ['gdb', 'tshark', 'radare2'],
    allowedWorkflows: [
      'web_triage',
      'host_service_enumeration',
      'web_dir_enum',
      'web_vuln_scan',
      'http_method_fuzz',
      'web_fetch',
      'web_shell_fetch',
      'pcap_grep_flag',
    ],
    deniedWorkflows: ['binary_triage'],
    allowShell: true,
    allowPython: false,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['crypto', 'file-forensics'],
    limits: { maxIterations: 80, maxToolCalls: 300 },
  },
  traffic: {
    id: 'traffic',
    displayName: 'Network Traffic Agent',
    description:
      '流量分析 Agent:pcap 解析( tshark / tcpflow )+ 过滤导出( tcpdump )+ HTTP/TLS/对象提取 + 跨协议接力。',
    systemPromptModules: ['role.boundary', 'tool.first', 'traffic.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'load_skill',
      'memory_search',
      'memory_recall',
      'emit_finding',
      'request_handoff',
      'extract_artifact',
      'tshark',
      'tcpdump',
      'tcpflow',
      'file',
      'strings',
      // §Round-3 — pcap_grep_flag workflow uses grep_for_flag tool.
      'grep_for_flag',
      'list_artifacts',
      'list_findings',
      'list_jobs',
      'query_background_job',
      'collect_background_result',
      'inspect_artifact_summary',
      'inspect_finding',
    ],
    deniedTools: ['nmap', 'sqlmap', 'gdb', 'radare2'],
    allowedWorkflows: ['pcap_triage', 'pcap_object_export', 'tcp_follow', 'pcap_grep_flag'],
    deniedWorkflows: ['host_service_enumeration'],
    allowShell: true,
    allowPython: false,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['file-forensics', 'crypto', 'reverse'],
    limits: { maxIterations: 60, maxToolCalls: 250 },
  },
  encoding: {
    id: 'encoding',
    displayName: 'Encoding Agent',
    description: '编码/解码 Agent:自动识别编码类型并递归解码。优先 encoding_sweep workflow。',
    systemPromptModules: ['role.boundary', 'tool.first', 'encoding.protocol'],
    allowedTools: [
      'Read', 'Glob', 'Grep', 'Bash', 'TodoWrite',
      'load_skill', 'memory_search', 'memory_recall',
      'Python', 'emit_finding', 'request_handoff', 'extract_artifact',
      'base64_decode', 'base64_encode', 'hex_decode', 'hex_encode',
      'url_decode', 'url_encode', 'response_diff', 'decode_tree',
      'xor_single_byte', 'xor_known_plaintext', 'atbash',
      'list_artifacts', 'list_findings', 'list_jobs',
      'query_background_job', 'collect_background_result',
      'inspect_artifact_summary', 'inspect_finding',
    ],
    deniedTools: ['nmap', 'sqlmap', 'gdb', 'tshark', 'radare2'],
    allowedWorkflows: ['encoding_sweep', 'classical_cipher_sweep', 'xor_key_search'],
    deniedWorkflows: ['host_service_enumeration'],
    allowShell: true,
    allowPython: true,
    allowBackgroundJobs: true,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: ['crypto', 'file-forensics'],
    limits: { maxIterations: 40, maxToolCalls: 150 },
  },
  misc: {
    id: 'misc',
    displayName: 'Misc Agent',
    description:
      '杂项 / 未分类兜底 Agent。沙箱里的安全子集 + Bash + Python,避免其它 Agent 工具不足时无路可走。',
    systemPromptModules: ['role.boundary', 'tool.first', 'handoff.protocol', 'artifact.protocol'],
    allowedTools: [
      'Read',
      'Glob',
      'Grep',
      'Bash',
      'TodoWrite',
      'load_skill',
      'memory_search',
      'memory_recall',
      'Python',
      'emit_finding',
      'request_handoff',
      'extract_artifact',
      'file',
      'strings',
      'binwalk',
      'exiftool',
      'openssl-rsa',
      'rot13',
      'morse_decode',
      'bacon_cipher',
      'vigenere',
      'multi_byte_xor',
      'rail_fence',
      'base85',
      'binary_decode',
      'braille',
      'list_artifacts',
      'list_findings',
      'list_jobs',
      'query_background_job',
      'collect_background_result',
      'inspect_artifact_summary',
      'inspect_finding',
    ],
    deniedTools: ['nmap', 'sqlmap', 'gdb', 'tshark', 'radare2'],
    allowedWorkflows: ['unknown_file_triage'],
    deniedWorkflows: [],
    allowShell: true,
    allowPython: true,
    allowBackgroundJobs: false,
    allowAgentHandoff: true,
    preferredAgentsForHandoff: [
      'encoding',
      'image-stego',
      'crypto',
      'file-forensics',
      'reverse',
      'pwn',
      'web',
      'traffic',
    ],
    limits: { maxIterations: 40, maxToolCalls: 150 },
  },
}

export function getBuiltinProfile(id: string): CapabilityProfile | undefined {
  return PROFILES[id]
}

export function listBuiltinProfiles(): string[] {
  return [...Object.keys(PROFILES)]
}