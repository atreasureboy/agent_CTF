...
### H7 · `scratch/` 76 个 test_proj_* 目录，是 phase31Wiring 测试无清理 hook 的直接产物
- **实测**：
  ```
...
- **修复**：
  ```ts
  afterAll(async () => { rmSync(tmpDir, { recursive: true, force: true }) })
  ```
...
- **证据**：
  ...
  runOneShot: async ({ manifestId, inputArtifactIds, options }) => {
    const { runOneShot } = this as unknown as {
      runOneShot?: (
        input: { manifestId: string; inputArtifactIds: string[]; options?: unknown },
...
- **证据**：
  ```ts
  try { reduceInternal(state, event) } catch { continue }
  ```
