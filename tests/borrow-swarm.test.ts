...
function makeMember(
  id: string,
  opts: { delayMs?: number; flag?: string; succeed?: boolean },
): {
  id: string
...
describe('SolverSwarm (B)', () => {
  ...
  it('returns failed when all members fail', async () => {
    const swarm = createSolverSwarmExecutor({
      members: [makeMember('a', { succeed: false }), makeMember('b', { succeed: false })],
      isFlagCheck: () => true,
      winnerTimeoutMs: 100,
    })
    const r = await swarm.execute(makeCtx(FLAG_ACTION))
    ...
    expect(
      r.status === 'failed' ||
        (r.status === 'executed' && r.materializedResult.flagCandidateDrafts.length === 0),
    ).toBe(true)
  })
  ...
  it('treats non-flag actions as ordinary racing', async () => {
    const swarm = createSolverSwarmExecutor({
      members: [makeMember('a', { delayMs: 5 }), makeMember('b', { delayMs: 10 })],
